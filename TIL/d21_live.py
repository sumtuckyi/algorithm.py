...
포화 이진 트리 : 모든 레벨에 노드가 포화상태로 차있는 이진 트리
완전 이진 트리 : 
편향 이진 트리
트리 자료구조의 데이터를 어떻게 탐색할 것인가? -> 트리를 배열로 표현, 연결리스트로 표현
이진 탐색 트리 : 어떤 경우에도 저장된 값이 왼쪽 서브트리의 루트 < 현재 노드 < 오른쪽 서브트리의 루트인 규칙을 만족하는 트리
'''
...
def postorder(n):
    if n:  # 정점이 존재하면
        preorder(ch1[n])  # 해당 정점의 왼쪽 자식 노드를 확인
        preorder(ch2[n])  # 해당 정점의 오른쪽 자식 노드를 확인
        print(n)
